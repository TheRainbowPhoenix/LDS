...
/**
 * Phaser.Pointer (Bridge Version)
 * Adapts RMMV TouchInput data into a Phaser Pointer structure.
 */
Phaser.Pointer = function (game, id, pointerMode=undefined) {
    /**
    ...
    // Position
    this.rawMovement = new Phaser.Point(); // Delta
...
Phaser.Pointer.prototype = {
    ...
    /**
     * Called by Phaser.Input.update to sync with RMMV TouchInput
     */
    updateFromTouchInput: function() {
        var prevX = this.x;
        var prevY = this.y;

        // 1. Sync Position from RMMV
        this.x = TouchInput.x;
        this.y = TouchInput.y;
        this.clientX = this.x; // RMMV normalizes this to canvas coords
        this.clientY = this.y;
        this.position.set(this.x, this.y);

        // 2. Calculate World Position based on Phaser Camera
        if (this.game.camera) {
            this.worldX = this.game.camera.view.x + this.x;
            this.worldY = this.game.camera.view.y + this.y;
        }

        // 3. Sync Button State
        var isCurrentlyDown = TouchInput.isPressed();

        // State Change: DOWN
        if (isCurrentlyDown && this.isUp) {
            this.isDown = true;
            this.isUp = false;
        ...
        // State Change: UP
        else if (!isCurrentlyDown && this.isDown) {
            this.isDown = false;
            this.isUp = true;
            this.timeUp = Date.now();
            // Don't set active false immediately to allow click processing
        }

        // 4. Calculate Movement
        this.rawMovement.set(this.x - prevX, this.y - prevY);
    }
};
