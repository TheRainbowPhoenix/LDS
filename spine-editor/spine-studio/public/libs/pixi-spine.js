...
(function (pixi_spine) {
    ...
    (function (core) {
        var AnimationState = (function () {
                ...
                for (var i = 0, n = tracks.length; i < n; i++) {
                    ...
                    if (next != null) {
                        ...
                        if (nextTime >= 0) {
                            ...
                            this.timeScale = next.nextTimeScale;
            ...
            AnimationState.prototype.setAnimation = function (trackIndex, animationName, loop, _Speed) {
                var _Speed = typeof _Speed !== 'undefined' ? _Speed : 1;
                var animation = this.data.skeletonData.findAnimation(animationName);
                if (animation == null) { //nupu:見つからない時は、処理をしない※エラーにならないようにする
                    console.log("モーション：" + animationName + "は見つかりませんでした")
                    return;
                }
                // if (animation == null){}
                //     throw new Error("Animation not found: " + animationName);
                return this.setAnimationWith(trackIndex, animation, loop, _Speed);
            };
            //nupu:_Speedを追加
            AnimationState.prototype.setAnimationWith = function (trackIndex, animation, loop, _Speed) {
                var _Speed = typeof _Speed !== 'undefined' ? _Speed : 1;
                if (animation == null)
                ...
                entry.timeScale = _Speed; //nupu：再生速度を対応
                this.setCurrent(trackIndex, entry, interrupt);
                this.queue.drain();
                return entry;
            };
            //nupu:_Speedを追加
            AnimationState.prototype.addAnimation = function (trackIndex, animationName, loop, delay, _Speed) {
                var _Speed = typeof _Speed !== 'undefined' ? _Speed : 1;
                var animation = this.data.skeletonData.findAnimation(animationName);
                if (animation == null)
                    throw new Error("Animation not found: " + animationName);
                return this.addAnimationWith(trackIndex, animation, loop, delay, _Speed);
            };
            //nupu:_Speedを追加
            AnimationState.prototype.addAnimationWith = function (trackIndex, animation, loop, delay, _Speed) {
                var _Speed = typeof _Speed !== 'undefined' ? _Speed : 1;
                if (animation == null)
                    throw new Error("animation cannot be null.");
                ...
                entry.nextTimeScale = _Speed; //nupu:次再生自のタイムスケール調整
                entry.delay = delay;
                return entry;
            };
    ...
    (function (core) {
        var Skeleton = (function () {
            ...
            Skeleton.prototype.setSkinByName = function (skinName) {
                //nupu:AddSkin対応 ■■■■■■■■■■■■■
                var newSkin = new PIXI.spine.core.Skin("combined-skin");
                for (i = 0; i <= $gameSystem.SpineSkinApr.length - 1; i++) {
                    if ($gameSystem.SpineSkinApr[i]._MdlName == this.Mdlname) {
                        newSkin.addSkin(this.data.findSkin($gameSystem.SpineSkinApr[i]._SkinName));
                    }
                }
                // for(i = 0; i <= SpineAddSkin.length - 1; i++){
                //     if (this.data.findSkin(SpineAddSkin[i]) != null) {
                //         newSkin.addSkin(this.data.findSkin(SpineAddSkin[i]));
                //     }else{
                //         console.log("ttt");
                //     }
                // }
