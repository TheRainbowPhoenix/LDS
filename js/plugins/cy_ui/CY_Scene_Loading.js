...
CY_Scene_Loading.prototype.create = function () {
    ...
    this.createUILayer();
    this._maxDuration = 360; // 6 seconds
...
CY_Scene_Loading.prototype.createProgressBar = function () {
    var redColor = parseInt(CY_System.Colors.lightRed.replace('#', ''), 16);
    var w = Graphics.width;

    // Position to the left of spinner (spinner x is w-90)
    var barWidth = 280;
    var barHeight = 12;
    var x = w - 90 - 45 - barWidth; // 45px padding from spinner center 
    var y = 80 - (barHeight / 2);
...
CY_Scene_Loading.prototype.update = function () {
    ...
    if (this._progressBarFill) {
        if (this._duration > 0) {
            ...
            // Add some noise/stutter to fake real loading
            // Only update sometimes or add random blocks?
            // Simple lerp for now
            this._progressBarFill.scale.x = progress;
        } else {
            this._progressBarFill.scale.x = 1;
        }
    }
