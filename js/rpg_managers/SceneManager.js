...
SceneManager._isUsableTimeInMs =
...
if (SceneManager._isUsableTimeInMs)
...
SceneManager.initElectron = function () {
  if (Utils.isElectron()) {
    // Electron-specific initialization can be added here
  }
};

SceneManager.initTauri = function () {
  if (Utils.isTauri()) {
    // Center the Tauri window on launch
    window.__TAURI__.core.invoke("center_window").catch(function (err) {
      // Silent fail if centering is not available
    });
  }
};
