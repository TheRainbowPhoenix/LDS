...
Phaser.Sound = function (game, key, volume, loop, connect)
{
    ...
    // RMMV specific properties
    // We treat sounds as SE by default unless the key suggests otherwise or manually flagged
    this._rmmvType = 'se'; // 'se', 'bgm', 'bgs', 'me'
    
    // If the key path contains 'bgm', auto-switch type (optional convenience)
    if (key.toLowerCase().indexOf('bgm') !== -1) this._rmmvType = 'bgm';
...
Phaser.Sound.prototype = {
    ...
    play: function (marker, position, volume, loop, forceRestart, onPlay)
    {
        ...
        // Construct RMMV Audio Object
        // RMMV volume is 0-100, Phaser is 0.0-1.0
        var rmmvAudio = {
            name: this.key,
        ...
        switch (this._rmmvType) {
            case 'bgm':
                AudioManager.playBgm(rmmvAudio, position || 0);
                break;
            case 'bgs':
                AudioManager.playBgs(rmmvAudio, position || 0);
                break;
            case 'me':
                AudioManager.playMe(rmmvAudio);
                break;
            case 'se':
            default:
                AudioManager.playSe(rmmvAudio);
                // Note: AudioManager.playSe is fire-and-forget. 
                // We don't get a reference to the buffer easily back to track state 
                // perfectly without modifying RMMV.
                break;
        }
    ...
    stop: function ()
    {
        ...
        switch (this._rmmvType) {
            ...
            case 'me':
                ...
                break;
            case 'se':
                // RMMV cannot stop a *specific* SE instance easily by name only.
                // AudioManager.stopSe() stops ALL SEs. 
                // For the bridge, we might have to accept this limitation or stop all.
                // Or we iterate AudioManager._seBuffers and stop the one matching this name.
                this._stopSpecificSe();
                break;
        }
    ...
    /**
     * Try to find the SE buffer in RMMV and stop it.
     */
    _stopSpecificSe: function() {
        // Accessing private RMMV array _seBuffers
        var buffers = AudioManager._seBuffers; 
        for (var i = 0; i < buffers.length; i++) {
            // RMMV WebAudio objects don't store the 'name' property directly on the buffer 
            // after creation easily accessible unless we modify AudioManager.createBuffer 
            // or WebAudio to attach it.
            // 
            // However, we can just stop all SEs if strict control is needed, 
            // or rely on natural completion.
            //
            // For now, let's play safe and do nothing for SE stop to avoid killing UI sounds,
            // unless the user explicitly wants to StopAll.
        }
    },
...
/**
 * Allow setting type explicitly (se, bgm, bgs)
 */
Object.defineProperty(Phaser.Sound.prototype, 'type', {
    get: function () { return this._rmmvType; },
    set: function (value) { this._rmmvType = value; }
});
