...
// Utils
Creature.Utils = Creature.Utils || {};

Creature.Utils.setAxisMatrix = function(xAxis, yAxis, zAxis)
{
...
Creature.Utils.matrixToQuat = function(mat_in)
{
...
Creature.Utils.rotateVec_90 = function(vec_in)
{
...
Creature.Utils.calcRotateMat = function(vec_in)
{
	var dir = vec3.clone(vec_in);
	dir = vec3.normalize(dir, dir);
	
	var pep_dir = Creature.Utils.rotateVec_90(dir);
	...
	cur_rotate = Creature.Utils.setAxisMatrix(cur_tangent, cur_normal, cur_binormal);
	
	return cur_rotate;
};

Creature.Utils.getMatTranslate = function(mat_in)
{
...
Creature.Utils.addMat = function(mat1, mat2)
{
	var retMat = mat4.create();
	for(var i = 0; i < 16; i++)
	{
...
Creature.Utils.mulMat = function(mat_in, factor)
{
	var retMat = mat4.create();
	for(var i = 0; i < 16; i++)
	{
...
Creature.Utils.clamp = function(num, min, max) {
...
Creature.Utils.vecInterp = function(vec1, vec2, ratio)
{
...
Creature.Utils.vec2Interp = function(vec_1, vec_2, ratio)
{
...
Creature.Utils.ptsInterp = function(src_pts, target_pts, fraction)
{
    var ret_pts = [];
    for (var i = 0; i < src_pts.length; i++)
    {
        ret_pts.push(Creature.Utils.vec2Interp(src_pts[i], target_pts[i], fraction));
    }

    return ret_pts;
};

Creature.Utils.scalarInterp = function(src_val, target_val, fraction)
{
...
//  var world_rest_dir = getWorldRestEndPt().cpy().sub( getWorldRestStartPt());
  ...
  var bind_rotate = Creature.Utils.calcRotateMat(tVec);
...
MeshBone.prototype.computeParentTransforms = function()
{
  ...
  var rotate_parent = Creature.Utils.calcRotateMat(tVec);
...
//  world_delta_mat = (cur_translate.cpy().mul(cur_rotate)).mul(bind_world_inv_mat);
  ...
  var cur_quat = Creature.Utils.matrixToQuat(this.world_delta_mat);


  var tmp_pos =  Creature.Utils.getMatTranslate(this.world_delta_mat);
...
MeshBone.prototype.computeDirs = function(start_pt, end_pt)
{
  ...
  var normal = Creature.Utils.rotateVec_90(tangent);
...
MeshBoneCacheManager.prototype.getIndexByTime = function(time_in)
{
  ...
  retval = Creature.Utils.clamp(retval, 0, (this.bone_cache_table.length) - 1);
...
MeshBoneCacheManager.prototype.retrieveValuesAtTime = function(time_in, bone_map)
{
  ...
  for(var i = 0, l = base_cache.length; i < l; i++) {
    ...
    var final_world_start_pt = Creature.Utils.vecInterp(base_data.getWorldStartPt(), end_data.getWorldStartPt(), ratio);

    var final_world_end_pt = Creature.Utils.vecInterp(base_data.getWorldEndPt(), end_data.getWorldEndPt(), ratio);
...
{
  var retval = time_in - this.start_time;
  retval = Creature.Utils.clamp(retval, 0, (this.displacement_cache_table.length) - 1);
  ...
  for(var i = 0; i < base_cache.length; i++) {
    ...
    if(set_region.getUseLocalDisplacements()) {
          ...
          && (end_data.getLocalDisplacements().length == displacements.length))
      {
        for(var j = 0; j < displacements.length; j++) {
          var interp_val = Creature.Utils.vec2Interp(base_data.getLocalDisplacements()[j], 
    ...
    if(set_region.getUsePostDisplacements()) {
          ...
          && (end_data.getPostDisplacements().length == displacements.length))
      {
        ...
        for(var j = 0; j < displacements.length; j++) {
          var interp_val = Creature.Utils.vec2Interp(base_data.getPostDisplacements()[j], 
...
{
  var retval = time_in - this.start_time;
  retval = Creature.Utils.clamp(retval, 0, (this.uv_cache_table.length) - 1);
...
{
  var retval = time_in - this.start_time;
  retval = Creature.Utils.clamp(retval, 0, (this.opacity_cache_table.length) - 1);
		...
		for (var j = 1; j < gap_diff; j++)
		{
			...
			for (var k = 0; k < cache_list.length; k++)
			{
				...
				gap_cache_data.setWorldStartPt(
					Creature.Utils.vecInterp(prev_data.getWorldStartPt(), cur_data.getWorldStartPt(), gap_fraction));
				gap_cache_data.setWorldEndPt(
					Creature.Utils.vecInterp(prev_data.getWorldEndPt(), cur_data.getWorldEndPt(), gap_fraction));
			...
			for (var k = 0; k < cache_list.length; k++)
			{
				...
				if (cur_data.getLocalDisplacements().length > 0)
				{
					gap_cache_data.setLocalDisplacements(
						Creature.Utils.ptsInterp(prev_data.getLocalDisplacements(), cur_data.getLocalDisplacements(), gap_fraction));
				}
				else {
					gap_cache_data.setPostDisplacements(
						Creature.Utils.ptsInterp(prev_data.getPostDisplacements(), cur_data.getPostDisplacements(), gap_fraction));
				}
	...
	if (gap_diff > 1)
	{
		...
		for (var j = 1; j < gap_diff; j++)
		{
			...
			for (var k = 0; k < cache_list.length; k++)
			{
				...
				gap_cache_data.setOpacity(Creature.Utils.scalarInterp(prev_data.getOpacity(), cur_data.getOpacity(), gap_fraction));
...
{
  var retval = time_in - this.start_time;
  retval = Creature.Utils.clamp(retval, 0, (this.cache_pts.length) - 1);
